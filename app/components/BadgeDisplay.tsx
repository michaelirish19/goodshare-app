...
function daysSince(seconds: number) { return (now() - seconds) / 86400; }

function getTierStyle(tier: Badge["tier"]) {
  switch (tier) {
    case "onboarding": return { bg: "linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%)", border: "#6ee7b7", text: "#065f46", glow: "0 0 8px rgba(16, 185, 129, 0.3)" };
    case "activity": return { bg: "linear-gradient(135deg, #fef9c3 0%, #fde68a 100%)", border: "#fcd34d", text: "#78350f", glow: "0 0 8px rgba(251, 191, 36, 0.3)" };
    case "trust": return { bg: "linear-gradient(135deg, #dcfce7 0%, #bbf7d0 100%)", border: "#86efac", text: "#14532d", glow: "0 0 8px rgba(34, 197, 94, 0.3)" };
    case "reach": return { bg: "linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%)", border: "#c4b5fd", text: "#4c1d95", glow: "0 0 8px rgba(139, 92, 246, 0.3)" };
    case "community": return { bg: "linear-gradient(135deg, #fce7f3 0%, #fbcfe8 100%)", border: "#f9a8d4", text: "#831843", glow: "0 0 8px rgba(236, 72, 153, 0.3)" };
  }
}

function getCertifiedStyle() {
  return { bg: "linear-gradient(135deg, #fef3c7 0%, #fde68a 50%, #fbbf24 100%)", border: "#f59e0b", text: "#3a2e00", glow: "0 0 12px rgba(245, 158, 11, 0.5)" };
}

export default function BadgeDisplay({ recommenderId, recommenderData, recommendationCount }: Props) {
  ...
  const {
    name, role, description, createdAt, lastActiveAt,
  ...
  // ── Onboarding ──
  if (isBetaUser) earnedBadges.push({ id: "founding_sharer", label: "Founding Sharer", description: "Joined GoodShare during beta", emoji: "🌱", tier: "onboarding" });
  if (recommendationCount >= 1) earnedBadges.push({ id: "first_pick", label: "First Pick", description: "Added your first pick", emoji: "📌", tier: "onboarding" });
  if (name?.trim() && role?.trim() && description?.trim()) earnedBadges.push({ id: "profile_complete", label: "Profile Complete", description: "Filled out your full profile", emoji: "✅", tier: "onboarding" });
  if (recommendationCount >= 1 && totalOutboundClickCount >= 1 && totalRatings >= 1) earnedBadges.push({ id: "first_steps", label: "First Steps", description: "Added a pick, got a click, and earned a rating", emoji: "🚀", tier: "onboarding" });
  ...
  if (lastActiveSeconds > 0 && daysSince(createdSeconds) >= 30 && daysSince(lastActiveSeconds) <= 30) earnedBadges.push({ id: "consistent", label: "Consistent", description: "Active for 30 consecutive days", emoji: "🔄", tier: "activity" });
  ...
  // ── Follower badges ──
  if (followerCount >= 1) earnedBadges.push({ id: "first_follower", label: "First Follower", description: "Someone followed you for the first time", emoji: "👥", tier: "community" });
  if (followerCount >= 10) earnedBadges.push({ id: "rising", label: "Rising", description: "10 followers", emoji: "📈", tier: "community" });
  if (followerCount >= 50) earnedBadges.push({ id: "known", label: "Known", description: "50 followers", emoji: "🔆", tier: "community" });
  if (followerCount >= 100) earnedBadges.push({ id: "respected", label: "Respected", description: "100 followers", emoji: "💫", tier: "community" });
  if (followingCount >= 10) earnedBadges.push({ id: "connected", label: "Connected", description: "Following 10 sharers", emoji: "🔗", tier: "community" });
  ...
  useEffect(() => {
    ...
    const profileRef = doc(db, "recommenders", recommenderId);
    updateDoc(profileRef, { seenBadges: arrayUnion(...earnedIds) }).catch(console.error);
    const timer = setTimeout(() => setShowToast(false), 4000);
    return () => clearTimeout(timer);
  }, [isOwner, loadingClicks, seenBadges.length, earnedBadges.length]);

  // ── Initialize seenBadges ──
  useEffect(() => {
    if (!isOwner || loadingClicks || seenBadges.length > 0 || earnedBadges.length === 0) return;
    const earnedIds = earnedBadges.map((b) => b.id);
    const profileRef = doc(db, "recommenders", recommenderId);
    updateDoc(profileRef, { seenBadges: earnedIds }).catch(console.error);
  ...
  const sorted = [...earnedBadges].sort((a, b) => tierOrder.indexOf(a.tier) - tierOrder.indexOf(b.tier));

  return (
    <>
      <div className="mt-5 border-t border-gray-100 pt-4">
        <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-gray-400">Badges</p>
            ...
            return (
              <div key={badge.id} className="group relative">
                <span
                  ...
                  style={{ background: style.bg, border: `1.5px solid ${style.border}`, color: style.text, boxShadow: style.glow }}
                >
                  <span className="text-sm">{badge.emoji}</span>
                  {badge.label}
                  {isNew && <span className="ml-0.5 inline-flex h-2 w-2 rounded-full bg-red-500" />}
                </span>
                <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 rounded-xl border border-gray-100 bg-white px-3 py-2 text-center shadow-lg group-hover:block">
                  <p className="whitespace-nowrap text-xs font-semibold text-gray-900">{badge.emoji} {badge.label}</p>
                  <p className="mt-0.5 whitespace-nowrap text-xs text-gray-500">{badge.description}</p>
                </div>
              </div>
            );
          })}
      ...
      {showToast && newBadge && (
        <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 animate-bounce" style={{ animationDuration: "0.6s", animationIterationCount: 2 }}>
          <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-5 py-4 shadow-2xl">
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-yellow-100 text-xl">{newBadge.emoji}</div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">Badge unlocked!</p>
